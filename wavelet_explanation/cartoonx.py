...
def run_with_config(
    ...
    config_path: str,
    save_path: str = 'cartoonx_result.png',
    show_plot: bool = True,
):
    """Run CartoonX once using a YAML config file."""
    device = 'cuda' if torch.cuda.is_available() else 'cpu'

    cfg = CartoonXConfig.from_yaml(config_path)

    # CLI image path overrides config image_path when provided.
    resolved_image_path = image_path or cfg.image_path
    if not resolved_image_path:
        raise ValueError(
            "No image path provided. Set image_path in YAML config or pass it on CLI."
        )

    if not os.path.exists(resolved_image_path):
        raise FileNotFoundError(
            f"Image path not found: {resolved_image_path}\n"
            "If running with --config, do not pass a placeholder positional image path.\n"
            "Use either:\n"
            "  python cartoonx.py --config <config.yaml>\n"
            "or:\n"
            "  python cartoonx.py <real_image_path> --config <config.yaml>"
        )

    x = load_image(resolved_image_path, size=cfg.image_size).to(device)

    base_model = models.vgg16(weights=models.VGG16_Weights.IMAGENET1K_V1)
    model = NormalizedModel(base_model).to(device).eval()

    explainer = CartoonX(model, cfg, device=device)
    explanation, mask, history = explainer.explain(x)

    if show_plot:
        explainer.visualize(x, explanation, mask=mask, save_path=save_path)
        if history:
            explainer.plot_loss_history(history)
    else:
        plt.imsave(save_path, explanation[0, 0].detach().cpu().numpy(), cmap='gray')
        print(f"Saved explanation to {save_path}")

    explainer.cleanup()
    return explanation, mask, history


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Run CartoonX explanations with preset demo mode or YAML config mode.'
    )
    parser.add_argument(
        'image_path',
        nargs='?',
        default=None,
        help='Path to input image. Optional in --config mode if YAML has image_path.'
    )
    parser.add_argument(
        '--config',
        type=str,
        default=None,
        help='Path to YAML config. If set, runs a single config-driven explanation.'
    )
    parser.add_argument(
        '--save-path',
        type=str,
        default='cartoonx_result.png',
        help='Output image path for config mode (default: cartoonx_result.png)'
    )
    parser.add_argument(
        '--no-plot',
        action='store_true',
        help='Disable interactive plotting and only save the explanation image.'
    )
    parser.add_argument(
        '--example',
        action='store_true',
        help='Run the original multi-config demo pipeline.'
    )
    args = parser.parse_args()

    if args.example:
        run_example(args.image_path or 'your_image.jpg')
    elif args.config:
        run_with_config(
        ...
        parser.error('Provide --config <yaml_path> for config mode, or use --example.')
